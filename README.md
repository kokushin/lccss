...
LCCSS（ルックス）は、各要素のカテゴライズを「レイアウト」と「コンポーネント」の2軸に絞ることで構造をシンプルにし、従来のCSS設計における思考時間を少しでも減らすことを目標としている。

- 各要素は `Layout` または `Component` のどちらかに属する。細かくカテゴライズしない
- `Layout` は「枠組み」となる要素に対して定義
- `Component` は「Layout内に含まれている」要素群に対して定義
- 接頭辞によって各要素の役割を明確に
...
| 種類       | 接頭辞 | 補足                                                     | 必須 |
|------------|--------|----------------------------------------------------------|------|
| Layout     |   l-   | 外枠や内枠など、枠組みを形成する要素に対して記述         | true |
| Component  |   c-   | レイアウト内に含まれている要素群に対して記述             | true |
...
- Layout内に含まれている要素群に対して記述
- 共通で使いまわすことを前提とした要素群も該当する
- `Component` として記述した要素群は、単体でも動作するよう設計することを推奨
...
CSSカテゴライズはSMACSSを参考にしているが、LCCSSでは「ベース/エリアとなる枠組み（SMACSSではベース・レイアウトに該当）」をまとめて `Layout` として定義し、「レイアウト内に含まれている要素群（SMACSSではモジュールに該当）」を `Component` として定義する。また `Component` は、共通で使いまわすことを想定した要素群も該当し、単体でも動作するよう設計することを推奨している。

コンポーネント内のパーツ（ボタン等）は、色・フォントサイズなど僅かなデザインが共通であっても新規でスタイルを適用する。コンポーネントは単体で動作するべきであり、余計なクラスを増やさないため。

命名規則はMindBEMdingを参考にしているが、一部改変しており簡潔に記述することが可能。スタイルの打ち消しは状況に応じて可能だが、基本的に打ち消さない方が良い。また、セレクタのネストを行わず詳細度を一定に保たなければならない（Modifierは除く）。
...
Version 1.0.1 / Updated date: 2017/05/18
