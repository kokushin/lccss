...
CSSカテゴライズはSMACSSを参考にしているが、LCCSSでは「ベース/エリアとなる枠組み（SMACSSではベース・レイアウトに該当）」をまとめて `Layout` として定義し、「レイアウト内に含まれている要素群（SMACSSではモジュールに該当）」を `Component` として定義する。

また `Component` は、共通で使いまわすことを想定した要素群も該当し、単体でも動作するよう設計することを推奨している。
